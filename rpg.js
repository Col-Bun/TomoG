...
// SINGLE-LANGUAGE mode: never fall back to the OTHER language. Fallback to
// English (grey, bracketed) so the player knows it's untranslated — but never
// mixes Spanish and Japanese on-screen.
function rpgWordHas(w, lang) {
  if (!w) return false;
  if (lang === 'es') return !!(w.es && w.es !== '—');
  return !!(w.jp && w.jp !== '—');
}
function rpgWordText(w, lang) {
  if (!w) return '';
  if (lang === 'es') {
    if (w.es && w.es !== '—') return w.es;
    return w.en && w.en !== '—' ? '〈' + w.en + '〉' : '—';
  }
  // ja
  if (w.jp && w.jp !== '—') return w.jp;
  return w.en && w.en !== '—' ? '〈' + w.en + '〉' : '—';
}
...
// Words missing the current language are dropped so the labyrinth stays
// single-language (Spanish OR Japanese, never both).
function rpgThemedPool(type) {
  ...
  const all = (data.dictionary || [])
    .filter(w => w.type === type)
    .filter(w => rpgWordHas(w, lang));   // ← key: require active-lang text
  if (themes.length === 0) return all;
  const themed = all.filter(w => (w.tags||[]).some(t => themes.includes(t)));
  // Fallback to full (still single-language) pool if themed set is too small
...
function rpgChoices(correctWord, type, count = 4) {
  const lang = (data.rpg && data.rpg.lang) || 'ja';
  // Only pull decoys that have the active language — prevents EN-bracket spam
  const pool = (data.dictionary || [])
    .filter(w => w.type === type)
    .filter(w => rpgWordHas(w, lang));
...
function rpgSetLang(lang) {
  ensureRpgData();
  if (lang !== 'ja' && lang !== 'es') return;
  data.rpg.lang = lang;
  saveData();
  renderRpg();
}
...
function renderRpg() {
  ...
  if (!data.rpg.run) {
    ...
    // Count words that actually support each language so the user sees what's available
    const jaCount = (data.dictionary || []).filter(w => w.jp && w.jp !== '—').length;
    const esCount = (data.dictionary || []).filter(w => w.es && w.es !== '—').length;
    container.innerHTML = `<div class="rpg-intro glass-dark">
      <h3>🗾 言の葉の迷宮 · El Laberinto de las Palabras</h3>
      <p>A turn-based labyrinth of NPCs asking for objects and actions. No combat — just words.</p>
      <div class="rpg-lang-picker">
        <p class="rpg-lang-picker-title"><b>選べ一つだけ</b> — pick ONE language for this run (cannot be both):</p>
        <div class="rpg-lang-picker-row">
          <button class="btn-glossy rpg-lang-btn ${jaActive ? 'rpg-lang-btn-active' : ''}" onclick="rpgSetLang('ja')">
            <div class="rpg-lang-btn-flag">🇯🇵</div>
            <div class="rpg-lang-btn-name">日本語</div>
            <div class="rpg-lang-btn-meta">${jaCount} words ready</div>
          </button>
          <button class="btn-glossy rpg-lang-btn ${!jaActive ? 'rpg-lang-btn-active' : ''}" onclick="rpgSetLang('es')">
            <div class="rpg-lang-btn-flag">🇨🇴</div>
            <div class="rpg-lang-btn-name">Español</div>
            <div class="rpg-lang-btn-meta">${esCount} words ready</div>
          </button>
        </div>
        <p class="rpg-lang-picker-hint">Currently: <b>${jaActive ? '🇯🇵 日本語' : '🇨🇴 Español'}</b></p>
      </div>
      <button class="btn-glossy btn-green rpg-btn-big" onclick="rpgStartRun(1)">⚔️ Begin Run · ${jaActive ? '日本語で' : 'en Español'}</button>
...
function renderRpgInventory(L) {
  ...
  // Single-language mode: only show nouns that actually have the active language.
  const lang = data.rpg.lang;
  const nouns = (data.dictionary || [])
