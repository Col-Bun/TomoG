  ...
  function commit() {
    ...
    for (const n of notes) {
      ...
      const rawTags = [...n.tags, ...tagsFromColumn, ...(extraTag ? [extraTag] : [])]
        ...
        .filter(Boolean);
      // Dedup
      let tags = Array.from(new Set(rawTags)).slice(0, 12);
      ...
      try {
        if (typeof window.mergeAutoTags === 'function') {
          entry.tags = window.mergeAutoTags(entry.tags, entry);
        }
      } catch(e) { /* swallow */ }
      window.data.dictionary.push(entry);
      added++;
    }
